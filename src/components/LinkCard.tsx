...
export function LinkCard({ id, name, links, onAddLink, onDeleteLink }: LinkCardProps) {
        ...
        {(links ?? [])
          ...
          .map((link, idx) => (
            <li key={link.id > 0 ? link.id : `link-${idx}`} className="overflow-hidden">
              <div className="group flex h-16 min-w-85 shrink-0 items-center overflow-hidden rounded-[40px] border border-slate-50 bg-white shadow-[0_4px_20px_rgba(0,0,0,0.03)] transition-all duration-300 hover:-translate-y-0.5 hover:bg-slate-50 hover:shadow-[0_8px_25px_rgba(0,0,0,0.06)]">
                <a
                  href={link.url ?? "#"}
                  target="_blank"
                  rel="noreferrer"
